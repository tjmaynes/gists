using AutoMapper;
using BlockFlixter.Domain.Core.Entities;
using BlockFlixter.Domain.Core.Interfaces;
using MediatR;
using System.Text.Json.Serialization;

namespace BlockFlixter.Domain.Handlers.Admin;

public class UpdateMovieHandler : IRequestHandler<UpdateMovieRequest, MovieDTO>
{
    private readonly IMovieInventoryRepository _movieRepository;
    private readonly IMapper _mapper;

    public UpdateMovieHandler(
        IMovieInventoryRepository movieRepository,
        IMapper mapper
    )
    {
        _movieRepository = movieRepository;
        _mapper = mapper;
    }

    public async Task<MovieDTO> Handle(UpdateMovieRequest request, CancellationToken cancellationToken)
    {
        if (await _movieRepository.GetMovieById(request.Id) != null)
        {
            var result = await _movieRepository.UpdateMovie(request.Id, request.Movie);
            return _mapper.Map<MovieDTO>(result);
        }
        else
        {
            var result = await _movieRepository.AddNewMovie(request.Movie);
            return _mapper.Map<MovieDTO>(result);
        }
    }
}

public readonly struct UpdateMovieRequest : IRequest<MovieDTO>
{
    [JsonPropertyName("id")]
    public Guid Id { get; init; }
    [JsonPropertyName("movie")]
    public MovieDTO Movie { get; init; }
}
